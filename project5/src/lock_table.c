...
int deadlock_situation(lock_t* wait_lobj, trx_t* trx){
	...
	if(front_lobj == wait_lobj){
		//printf("It's Head!!!!\n");
	}
	while(front_lobj != wait_lobj){
		//printf("situ front_lobj - table : %d key : %d trx : %d mode : %d %d\n", front_lobj->sentinel->table_id, front_lobj->sentinel->record_id, front_lobj->owner_trx_id, front_lobj->lock_mode, front_lobj->is_wait);
		for(i = 0; i < TRX_NUM; i++){
		...
		if(front_lobj->owner_trx_id != wait_lobj->owner_trx_id){
			...
			while(inspect_lobj != NULL){
				//printf("inspect_lobj - table : %d key : %d trx : %d mode : %d %d\n", inspect_lobj->sentinel->table_id, inspect_lobj->sentinel->record_id, inspect_lobj->owner_trx_id, inspect_lobj->lock_mode, inspect_lobj->is_wait);
				...
				//if(inspect_lobj != NULL)
					//printf("nextinspect_lobj - table : %d key : %d trx : %d mode : %d %d\n", inspect_lobj->sentinel->table_id, inspect_lobj->sentinel->record_id, inspect_lobj->owner_trx_id, inspect_lobj->lock_mode, inspect_lobj->is_wait);
			}
		}
		front_lobj = front_lobj->next_lobj;
		//printf("nextsitu front_lobj - table : %d key : %d trx : %d mode : %d %d\n", front_lobj->sentinel->table_id, front_lobj->sentinel->record_id, front_lobj->owner_trx_id, front_lobj->lock_mode, front_lobj->is_wait);
	}
	return 0;
}
...
int deadlock_detect(lock_t* possible_deadlock){
	...
	//printf("possible_trx = %d, record = %d\n", possible_deadlock->owner_trx_id, possible_deadlock->sentinel->record_id);
	//printf("detect_trx = %d\n", self_trx->trx_id);
	entry = possible_deadlock->sentinel;
	front_lobj = entry->head;
	while(front_lobj != possible_deadlock){
		//printf("front_lobj - table : %d key : %d trx : %d mode : %d %d\n", front_lobj->sentinel->table_id, front_lobj->sentinel->record_id, front_lobj->owner_trx_id, front_lobj->lock_mode, front_lobj->is_wait);
		...
		front_lobj = front_lobj->next_lobj;
		//printf("nextfront_lobj - table : %d key : %d trx : %d mode : %d %d\n", front_lobj->sentinel->table_id, front_lobj->sentinel->record_id, front_lobj->owner_trx_id, front_lobj->lock_mode, front_lobj->is_wait);
	}
	//printf("unlock test!!!\n");
	pthread_mutex_unlock(&trx_table_latch);
	//printf("unlock after test!!!\n");
	return 0;
}
