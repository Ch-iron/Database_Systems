...
void frame_flush(pagenum_t page_num, buffer_st* drop){
	...
	/*for(k = 0; k < LOG_NUM; k++){
		if(log_buffer[k]->LSN == 0){
			break;
		}
		if(drop->table_id == log_buffer[k]->table_id && drop->page_num == log_buffer[k]->page_num){
			tmp = log_buffer[k]->LSN;
		}
	}*/
	log_write();
	//drop->frame->page_LSN = tmp;
